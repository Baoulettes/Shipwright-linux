        ...
        if (CVar_GetS32("gKeepMask", 0) != 0) {
            if (CVar_GetS32("gNoRestrictAge", 0) != 0) {
                if (gSaveContext.fileNum == 1) {
                    ...
                    this->currentMask = CVar_GetS32("gMaskNum_2", 0);
                } else if (gSaveContext.fileNum == 3) {
                    this->currentMask = CVar_GetS32("gMaskNum_3", 0);
                } else {
                    this->currentMask = CVar_GetS32("gMaskNum_debug", 0);
                }
            } else {
                if (gSaveContext.fileNum == 1 && LINK_AGE_IN_YEARS == LINK_IS_CHILD) {
                    ...
                    this->currentMask = CVar_GetS32("gMaskNum_3", 0);
                } else if (gSaveContext.fileNum != 1 && gSaveContext.fileNum != 2 && gSaveContext.fileNum != 3 && LINK_AGE_IN_YEARS == LINK_IS_CHILD) {
                    this->currentMask = CVar_GetS32("gMaskNum_debug", 0);
...
uintptr_t SelectedAnim = 0;
s16 EquipedStance;
s16 FrameCountSinceLastAnim = 0;
s16 MinFrameCount;
        ...
        if(SelectedMode == 18){
            ...
            if (FrameCountSinceLastAnim == 0) {
                ...
                if (SwitchAtFrame < MinFrameCount) {
                    ...
                    //Since we have more (or same) animation time than min duration we set the wait time to animation time.
